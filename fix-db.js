...
async function updateDB() {
  try {
    await mongoose.connect(mongoUri, { useNewUrlParser: true, unifiedTopology: true });
    ...
    if (product) {
      ...
      if (settings && settings.banners) {
        ...
        if (customBanner && customBanner.buttonLink === '/collection/storage') {
          console.log('Updating banner link to:', '/collection/' + product.slug);
          await bannerColl.updateOne({}, {
            $set: { 'banners.customBanner.buttonLink': '/collection/' + product.slug }
          });
          console.log('Banner updated successfully!');
        } else {
          console.log('Banner link is not /collection/storage, it is:', customBanner?.buttonLink);
          // Let's force update anyway just to be sure
          if (customBanner) {
            await bannerColl.updateOne({}, {
              $set: { 'banners.customBanner.buttonLink': '/collection/' + product.slug }
            });
